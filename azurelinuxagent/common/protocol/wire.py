...
class WireClient(object): # pylint: disable=R0904
    ...
    def __send_request_using_host_channel(self, host_func):
        ...
        if ret and not self.is_host_plugin_default_channel():
    ...
    def __send_request_using_direct_channel(self, direct_func):
        ...
        if ret and self.is_host_plugin_default_channel():
            # Set direct channel as default if it succeeded.
            self.set_host_plugin_default_channel(False)
    ...
    def send_request_using_appropriate_channel(self, direct_func, host_func):
        primary_ret = None
        try:
            primary_ret = self.__send_request_using_host_channel(host_func) if self.is_host_plugin_default_channel() \
        ...
        secondary_ret = self.__send_request_using_direct_channel(direct_func) if self.is_host_plugin_default_channel() \
